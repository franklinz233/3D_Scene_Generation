...
from scenedreamer.geometry.array_ops import visual_pcd
...
class RenderPreview():
    ...
    def _visual_pcd(self,scene:GaussianScene):
        xyzs,rgbs = [],[]
    ...
    @torch.no_grad()
    def _render_video(self,scene:GaussianScene,save_dir='./',colorize=False):
        save_dir = Path(save_dir)
        save_dir.mkdir(parents=True, exist_ok=True)
        # render 5times frames
        nframes = len(scene.frames)*25
        video_trajs = build_camera_path(None,scene,nframes=nframes)
        ...
        imageio.mimwrite(str(save_dir / 'video_rgb.mp4'),rgbs,fps=20)
        imageio.mimwrite(str(save_dir / 'video_dpt.mp4'),dpts,fps=20)
