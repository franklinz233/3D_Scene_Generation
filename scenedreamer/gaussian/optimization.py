...
class RGBLoss():
    def __init__(self,w_lpips=0.2,w_ssim=0.2, device=None):
    ...
    def __call__(self,pr,gt,valid_mask=None):
        ...
        if valid_mask is not None and not isinstance(valid_mask, torch.Tensor):
            valid_mask = torch.from_numpy(valid_mask).to(device=pr.device, dtype=torch.bool)
        elif valid_mask is not None:
            valid_mask = valid_mask.to(device=pr.device, dtype=torch.bool)
...
class GaussianSceneOptimizer():
    ...
    def __init__(self,
        ...
        self.rgb_lossfunc = RGBLoss(w_lpips=0, device=self.device)
    ...
    def __call__(self,target_frames=None):
        ...
        for _ in tqdm.tqdm(range(self.iters)):
            ...
            loss_rgb = self.rgb_lossfunc(render_rgb,self._to_device(frame.rgb),valid_mask=frame.inpaint)
            # optimization
            loss = loss_rgb
            loss.backward()  
            self.optimizer.step()
