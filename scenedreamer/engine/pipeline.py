"""Coarse-to-fine scene reconstruction pipeline."""
...
from scenedreamer.cameras import build_camera_path
from scenedreamer.diagnostics.render_preview import RenderPreview
from scenedreamer.gaussian.optimization import GaussianSceneOptimizer
from scenedreamer.gaussian.scene import Frame, GaussianScene
from scenedreamer.geometry.array_ops import save_pic
from scenedreamer.geometry.depth_blending import OcclusionRemoval
...
from scenedreamer.stages.depth_reconstruction import DepthReconstructionStage
from scenedreamer.stages.multiview_refinement import MultiViewRefinementStage
from scenedreamer.stages.rgb_completion import RGBCompletionStage
        
class SceneDreamerPipeline():
    def __init__(self,cfg) -> None:
        if not torch.cuda.is_available():
            raise RuntimeError('SceneDreamer currently requires a CUDA-capable GPU.')

        self.device = 'cuda'
        self.cfg = cfg
        self.sky_value = cfg.model.sky.value
        self.sky_segor = SkySegmenter(cfg)
        self.rgb_inpaintor = RGBCompletionStage(cfg)
        self.reconstructor = DepthReconstructionStage(cfg)
        # temp

        self.removalor = OcclusionRemoval()
        self.checkor = RenderPreview()
    ...
    def _resize_input(self,fn):
        ...
        backup_path = image_path.with_name(f'{image_path.stem}.original{image_path.suffix}')

        if backup_path.exists():
            source_path = backup_path
        else:
            source_path = image_path
            Image.open(image_path).convert('RGB').save(backup_path)

        rgb = np.array(Image.open(source_path).convert('RGB'))[:, :, :3] / 255.
        H,W = rgb.shape[0:2]
        if H>W:
    ...
    def _initialization(self,rgb):
        ...
        self.scene = GaussianSceneOptimizer(self.scene,iters=100)(self.scene.frames)
    
    def _generate_traj(self):
        self.dense_trajs = build_camera_path(self.cfg,self.scene)
    ...
    def _coarse_scene(self,rgb):
        ...
        for i in range(self.n_sample-2):
            ...
            self.scene = GaussianSceneOptimizer(self.scene,iters=self.opt_iters_per_frame)(self.scene.frames)

    def _MCS_Refinement(self):
        refiner = DiffusionRectifier(device='cuda',use_lcm=True,denoise_steps=self.mcs_iterations,
                             sd_ckpt=self.cfg.model.optimize.sd,
                             lcm_ckpt=self.cfg.model.optimize.lcm)
        self.MVDPS = MultiViewRefinementStage(self.scene,device='cuda',
                                         refiner=refiner,
    ...
    def __call__(self):
        ...
        rgb_path = Path(rgb_fn)
        if not rgb_path.exists():
            raise FileNotFoundError(f'Input image not found: {rgb_path}')

        output_dir = rgb_path.parent
        # temp_interval_image
        self.coarse_interval_rgb_fn = str(output_dir / 'temp.coarse.interval.png')
        self.refine_interval_rgb_fn = str(output_dir / 'temp.refine.interval.png')
        # coarse
        self.scene = GaussianScene(self.cfg)
        ...
        torch.save(self.scene, output_dir / 'scene.pth')
        self.checkor._render_video(self.scene, save_dir=output_dir)
