...
from scenedreamer.gaussian.optimization import GaussianSceneOptimizer, RGBLoss
from scenedreamer.gaussian.scene import Frame, GaussianScene
from scenedreamer.geometry.array_ops import save_pic

class MultiViewRefinementStage():
    def __init__(self,
                 coarse_GS:GaussianScene,
                 device = 'cuda',
                 refiner = None,
                 traj_type = 'spiral',
                 n_view = 8,
        ...
        # loss function
        self.rgb_lossfunc = RGBLoss(w_ssim=0.2, device=self.device)
    ...
    def _pre_process(self): 
        ...
        trajs = build_camera_path(None,self.coarse_GS,nframes=self.n_view+2)[1:-1]
    ...
    def _to_device(self,tensor):
        tensor = torch.from_numpy(tensor.astype(np.float32)).to(self.device)
        return tensor
    ...
    def _x0_rectification(self, denoise_rgb, iters):
        ...
        self.refine_GS = GaussianSceneOptimizer(CGS)
        # rectification
        for iter in range(iters):
            ...
            for i in range(2):
                ...
                loss_rgb = self.rgb_lossfunc(render_rgb,self._to_device(keep_frame.rgb),valid_mask=keep_frame.inpaint)
    ...
    def _step_gaussian_optimization(self,step):
        ...
        with torch.no_grad():
            ...
            rgb_t = rgb_t.reshape(1).to(self.device)
